# Yarr.It — architecture

The deep design notes behind [README.md](README.md).
...
refused so the relay cannot be used to reach the network behind it.

## Components

| Piece | Role |
|---|---|
| `bridge/` | WebSocket↔TCP/UDP byte relay, per-IP caps, monthly budget |
| `search/` | Prowlarr front-end; groups releases into title cards |
| `web/` | SPA, in-browser engine, service-worker player |
...
Registration order is `torrent, embed, flash, game, playlist, url`. `url` is the
catch-all and must stay last: a YouTube link and an `.m3u8` link are both
http(s) URLs, so the specific resolvers need first refusal.
...
`-budget-gib`) so continuous IPTV can never drain the month.
...
redirect to a private address and walk into the network behind it.
